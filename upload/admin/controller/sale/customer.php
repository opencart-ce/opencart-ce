...
class ControllerSaleCustomer extends Controller {
	...
	public function login() {
		...
		if ($customer_info) {
			...
			if ($store_info) {
				$this->redirect(($this->config->get('config_use_ssl') ? $store_info['ssl'] : $store_info['url']) . 'index.php?route=account/login&token=' . $token);
			} else {
				$this->redirect(($this->config->get('config_use_ssl') ? HTTPS_CATALOG : HTTP_CATALOG) . 'index.php?route=account/login&token=' . $token);
			}
		} else {
